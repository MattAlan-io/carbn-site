...
function getColClassnames(props: Props, extraClassName?: string) {
  ...
  const bootstrapClasses = gridSizes.map(size => `w-${size}-${props[size]}`);

  return [...bootstrapClasses, extraClassName].filter(isNotNullOrUndefined).join(' ');
}

const Col = ({ children, className, ...gridProps }: Props) => <div className={getColClassnames(gridProps, className)}>{children}</div>;
